...
contains
    ...
    subroutine metodo_dfp(fun, grad, x0, alpha, beta, gama, sigma, eps, maxits)
        ...
        do while (norm2(grad(xk)) .gt. eps) ! Critério de parada (precisão da norma do gradiente)
            ...
            if (dot_product(sk, yk) .gt. 0) then
                hk = hk + (dot_product(sk, sk)/dot_product(sk, yk)) - (dot_product(zk, zk)/dot_product(zk, yk))
                ! print *, hk
            else
                hk = hk
                ! print *, hk
...
5       format(1X, A, D24.17) ! Formatação do valor da função na solução
    end subroutine metodo_dfp

end module dfp_method
